...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for event in orm['nox.Event'].objects.all():
            event.started_at = event.created_at
            event.creator = event.invite_set.all()[:1].get().user
            event.save()

    def backwards(self, orm):
        print("This data migration doesn't ever need to be reversed. Hooray, no-op!")
