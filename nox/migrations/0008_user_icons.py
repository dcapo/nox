...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'CustomUser.icon'
        db.add_column('custom_user', 'icon',
                      self.gf('django.db.models.fields.files.ImageField')(max_length=100, null=True),
                      keep_default=False)

        # Adding unique constraint on 'PostDislike', fields ['post', 'user']
        db.create_unique('post_dislike', ['post_id', 'user_id'])

        # Adding unique constraint on 'Invite', fields ['user', 'event']
        db.create_unique('invite', ['user_id', 'event_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Invite', fields ['user', 'event']
        db.delete_unique('invite', ['user_id', 'event_id'])

        # Removing unique constraint on 'PostDislike', fields ['post', 'user']
        db.delete_unique('post_dislike', ['post_id', 'user_id'])

        # Deleting field 'CustomUser.icon'
        db.delete_column('custom_user', 'icon')
